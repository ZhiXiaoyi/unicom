...
<html>
  <head>
    ...
    <title>My JSP 'StatisticalModel.jsp' starting page</title>
  ...
  <body>
  	<div>
	   <form class="form-inline" role="form" action="cartogram/toAreaCartogram" method="post">
          ...
          <div class="row">
             ...
             <c:when test="${Checkbox1.equals(\"on\") }">
          	  <div class="box box-success">
          	  	<table class="table">
					<tr>
						<td>时间</td><td>地区</td><td>电话卡销量</td><td>充值</td><td>点子卡销量</td>
					</tr>
					<tr>
						<td>${startdate }~${lastdate }</td><td>${ area}</td><td>${sr.orders}</td><td>${sr.rechargeRecord}</td><td>${sr.prepaidcard}</td>
					</tr>
				</table>
          	  </div>
          	  </c:when>
  ...
  <script>
		...
		***/
        var areaChartData = {
          ...
          datasets: [
            ...
            },
            {
              ...
              data: ["${sr.orders}","${sr.rechargeRecord}","${sr.prepaidcard}"]
            },
            {
              data: []
            }
        ...
        var donutData = [
          {label: "充值", data: "${sr.rechargeRecord}", color: "#3c8dbc"},
          {label: "电子卡销售额", data:"${sr.prepaidcard}", color: "#0073b7"},
          {label: "电话卡销售额", data:"${sr.orders}", color: "#00c0ef"},       
        ];
